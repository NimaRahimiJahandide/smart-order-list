...
interface TableProps {
  ...
  totalPages: number;
  isInitial: boolean;
  isFetchingNext: boolean; // اسکلتونِ پایین جدول (صفحهٔ بعد)
  isFetchingPrev: boolean; // اسکلتونِ بالای جدول (صفحهٔ قبل)
...
export const OrdersTable: React.FC<TableProps> = ({
  ...
  // ── جلوگیری از «پرش» بصری وقتی ردیف‌های جدید بالای جدول اضافه می‌شوند ──
  ...
  useLayoutEffect(() => {
    ...
    if (pendingTopCompRef.current) {
      ...
      if (diff !== 0) {
        el.scrollTop += diff; // همان‌قدر که محتوا بالا اضافه شد، اسکرول را جلو می‌بریم
      }
      if (!isFetchingPrev) {
  ...
  // ── بازنشانی اسکرول وقتی فیلترها عوض شده و لیست از صفر ساخته می‌شود ───
  useEffect(() => {
  ...
  const EDGE = 300; // فاصلهٔ پیکسلی از لبه برای شروع پیش‌بارگذاری
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    // نزدیک انتهای جدول → اسکلتون پایین + لود صفحهٔ بعد
    if (!isFetchingNext && maxPage < totalPages) {
      if (scrollHeight - clientHeight - scrollTop < EDGE) {
        onLoadMore();
      }
    }

    // نزدیک ابتدای جدول → اسکلتون بالا + لود صفحهٔ قبل (بدون پرش)
  ...
  return (
    <div className="relative rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden flex flex-col">
      {/* بدون اورلیِ تمام‌صفحه؛ فقط یک نوار وضعیت سبک */}
        ...
        <table className="w-full text-right border-collapse">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {isInitial ? (
              ...
              <>
                {/* اسکلتون بالای جدول هنگام لود صفحهٔ قبل */}
                {isFetchingPrev && <SkeletonRows count={6} />}
                ...
                {/* اسکلتون پایین جدول هنگام لود صفحهٔ بعد */}
                {isFetchingNext && <SkeletonRows count={6} />}
              </>
            )}
          </tbody>
        </table>
