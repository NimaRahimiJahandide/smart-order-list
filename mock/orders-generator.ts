...
const FIRST_NAMES = ['John', 'Jane', 'Alex', 'Emily', 'Michael', 'Sarah', 'David', 'Jessica', 'James', 'Elena'];
const LAST_NAMES = ['Smith', 'Doe', 'Johnson', 'Williams', 'Brown', 'Jones', 'Miller', 'Davis', 'Garcia', 'Rodriguez'];
const DOMAINS = ['gmail.com', 'yahoo.com', 'outlook.com', 'proton.me', 'corporate.com'];
...
export function generateMockOrders(count = 250): Order[] {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const priority = ORDER_PRIORITIES[Math.floor(Math.random() * ORDER_PRIORITIES.length)];
    const itemsCount = Math.floor(Math.random() * 8) + 1;
    const totalAmount = parseFloat((itemsCount * (Math.random() * 45 + 5) + Math.random() * 15).toFixed(2));
    
    // Generate dates distributed across last 45 days
